...
export default async function getURLs(request) {
    ...
    What did the genius send the assistant?`;
	...
	const metaphorSearchPrompt = await chain.call({ userRequest: request });
	console.log(metaphorSearchPrompt);

	const url = "https://api.metaphor.systems/search";

	const data = {
		query: metaphorSearchPrompt.text,
		numResults: 10,
